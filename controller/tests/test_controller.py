...
def test_methylate_organism(mock_k8s_client):
    ...
    apps_v1.patch_namespaced_stateful_set.assert_called_once_with(
        "organism", NAMESPACE, expected_body
    )
...
def test_monitor_cells_deleted_event(mock_k8s_client, mocker):
    ...
    # Mock statefulset reading
    mock_statefulset = MagicMock()
    mock_statefulset.spec.template.metadata.annotations.get.return_value = "1"
    apps_v1.read_namespaced_stateful_set.return_value = mock_statefulset
...
def test_methylate_organism_deployment(mock_k8s_client, mocker):
    mocker.patch('controller.controller.ORGANISM_TYPE', 'deployment')
    _, apps_v1 = mock_k8s_client
    pod_name = "test-pod"
    current_level = "1"
    new_level = 2
...
def test_monitor_cells_deleted_event_deployment(mock_k8s_client, mocker):
    mocker.patch('controller.controller.ORGANISM_TYPE', 'deployment')
    v1, apps_v1 = mock_k8s_client
    ...
    mock_pod = MagicMock()
    mock_pod.status = MagicMock()
    mock_pod.metadata.annotations.get.return_value = "1"
